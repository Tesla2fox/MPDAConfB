...
def generateConRate(rateNum, rateType, posLst=[]):
    if rateType == VALUEDIS['UNITARY']:
        resLst = [200 for x in range(rateNum)]
    if rateType == VALUEDIS['SVLCV']:
        resLst = [random.uniform(100, 200) for x in range(rateNum)]
    if rateType == VALUEDIS['SVSCV']:
        resLst = [random.uniform(100, 150) for x in range(rateNum)]
    if rateType == VALUEDIS['LVLCV']:
        resLst = [random.uniform(150, 300) for x in range(rateNum)]
    if rateType == VALUEDIS['LVSCV']:
        resLst = [random.uniform(150, 200) for x in range(rateNum)]
    if rateType == VALUEDIS['QUADRANT']:
        resLst = []
        for pos in posLst:
            x_bias = pos.x - 50
            y_bias = pos.y - 50
            if x_bias * y_bias > 0:
                resLst.append(random.uniform(100, 150))
            else:
                resLst.append(random.uniform(150, 200))
    if rateType == VALUEDIS['MSVFLV']:
        svNum = math.ceil(random.uniform(0.7, 0.95) * rateNum)
        resLst = [random.uniform(100, 150) for x in range(svNum)]
        lvNum = rateNum - svNum
        lvLst = [random.uniform(150, 200) for x in range(lvNum)]
        resLst.extend(lvLst)
    return resLst
...
if __name__ == '__main__':
    ...
    numConfLst = []
    all_combConfLst = []
    numLst = list(range(min_robNum, 25, 3))
    ...
    for i in range(len(numLst)):
        ...
        for i in range(4):
            while True:
                ...
                conType = valueTypeLst[random.randint(0,6)]
                confTuple = (robPosType, taskPosType, robRateType, taskRateType,conType)
